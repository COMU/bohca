...
class Ui_MainWindow(QtGui.QMainWindow):
	...
	def check(self):
		...
		if user_password == "" or user_password == " ": # Checked whether user password is empty
			...
			user_direc_name = user_name + "-bohca"
			if user_direc_name:
				try:
					user_direc = bool(open(os.getenv('HOME')+ "/gitolite-admin/"+ user_direc_name+ "/"+ "kayit.txt","r"))
					ui.label_5.setText("<html><head/><body><p><span style=\" color:#ff0000;\">  You had done setup Bohca !</span></p></body></html>")		
			
				except IOError:

				# bu kısım web sayfasında kayıt sırasında yapılacak !
					os.mkdir(os.getenv('HOME')+ "/gitolite-admin/" + user_direc_name)  # directory is created at this line.
					file_user = open(os.getenv('HOME') + "/gitolite-admin/"+ user_direc_name+ "/"+ "kayit.txt","w")
